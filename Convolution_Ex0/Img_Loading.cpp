...
int main()
{
	...
	const int batch_size = 100; // 이미지 총 갯수
	vector<pair<Mat, string>> ImgBox; // 이미지 데이터, 이미지 이름
	ImgBox = TraverseFilesUsingDFS("C:\\cifar\\test1000");// 이미지가 저장되어 있는 폴더 경로
	...
	*/
	...
	for (int i = 0; i < batch_size; i++)
	{
		std::cout << "라벨 출력 :: " << setw(10) << ImgBox[i].second << "넘버링 출력 :: " << setw(3) << ImgBox2[i].second << "데이터 출력 :: ";
		for (int x = 0; x < 32; x++)
		{
			std::cout << setw(3) << input_vec[i][0][0][x];
		}
		std::cout << std::endl;
	}
