...
void ValueCheck(vector < vector<float>> ValueCheck_input) {

	int imgsize = ValueCheck_input.size()
	int fullyconnectedsize = ValueCheck_input[0].size()
...
vector<vector<float>> FullyConnected(int FullyConnectedSize, vector<vector<vector<vector<float>>>> fullyConnectedInput, vector<vector<vector<vector<float>>>> fullyConnectedWeight) {
	...
	for (int f = 0; f < ImageNumf ; f++)
	{
		FullyConnectedOutput[f].resize(FullyConnectedSize);
		for (int i = 0; i < FullyConnectedSize; i++)
		{
			float sum = 0;
			for (int c = 0; c < FeatureChf; c++)
			{
				for (int y = 0; y < FeatureHeightf; y++)
				{
					...
					{
						sum += fullyConnectedInput[i][c][y][x] * fullyConnectedWeight[i][c][y][x];
					}
				}
			}
			FullyConnectedOutput[f][i] = sum;
		}
	}
...
vector<vector<float>>  GeneratorFullyConnectedWeight(vector<float> FullyConnectedInput, int FullyConnectedSize) {
	...
	fullyConnectedWeight.resize(ImageNumf );
	for (int f = 0; f < ImageNumf; f++)
	{
